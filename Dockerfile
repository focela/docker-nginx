# Nginx Custom Build Container
# Maintainer: Focela Labs (https://www.focela.com)
# Purpose: Build custom Nginx with LDAP, Brotli, Headers More, and security modules
...
LABEL org.opencontainers.image.source="https://github.com/focela/docker-nginx/"
LABEL org.opencontainers.image.description="Custom Nginx build with LDAP, Brotli, and security modules"
...
ENV NGINX_VERSION=${NGINX_VERSION:-"1.28.0"} \
    ...
    IMAGE_REPO_URL="https://github.com/focela/docker-nginx/"
...
RUN source /assets/functions/00-container && \
    ...
    # ===================================================================
    \
    # Create nginx user with UID 80 (standard web server UID) \
    ...
    \
    # ===================================================================
    # NGINX MODULES DOWNLOAD
    # ===================================================================
    \
    # Clone third-party Nginx modules \
    ...
    # ===================================================================
    \
    # Configure Nginx build with custom modules and settings \
    ./configure --prefix=/etc/nginx \
      ...
      # GCC 11.2 compatibility fix - https://github.com/google/ngx_brotli/issues/124 \
      ...
      --with-threads \
      # HTTP/3 support (requires QUICHE library) \
#      --with-quiche=/usr/src/quiche \
#      --with-http_v3_module \
    && \
    \
    # Build Nginx using all available CPU cores \
    ...
    # ===================================================================
    \
    # Set up Nginx directory structure \
    ...
    \
    # ===================================================================
    # BAD BOT BLOCKER INSTALLATION
    # ===================================================================
    \
    # Create directories for bot blocking rules \
    mkdir -p /etc/nginx/snippets/blockbots && \
    mkdir -p /etc/nginx/snippets/blockbots-custom && \
    \
    # Download Mitchell Krog's Ultimate Bad Bot Blocker configuration files \
    ...
    \
    # Update paths in configuration files to match our directory structure \
    sed -i "s|/etc/nginx/bots.d/|/etc/nginx/snippets/blockbots/|g" /etc/nginx/snippets/blockbots/globalblacklist.conf && \
    # Point to custom configuration files for user overrides \
